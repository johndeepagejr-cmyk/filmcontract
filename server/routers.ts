...
export const appRouter = router({
  ...
  actorProfile: router({
    ...
    getVideos: publicProcedure
      .input(z.object({ userId: z.number() }))
      .query(async ({ input }) => {
        return db.getActorVideos(input.userId);
      }),
    ...
    addVideo: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        return db.addActorVideo(ctx.user.id, input);
      }),
    ...
    updateVideo: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        return db.updateActorVideo(videoId, ctx.user.id, data);
      }),
    ...
    deleteVideo: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        return db.deleteActorVideo(input.videoId, ctx.user.id);
      }),
    ...
    uploadVideo: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        // Convert base64 to buffer
        const base64WithoutPrefix = input.base64Data.replace(/^data:video\/\w+;base64,/, "");
        const buffer = Buffer.from(base64WithoutPrefix, "base64");
        
        // Generate unique filename
        const timestamp = Date.now();
        const extension = input.fileName.split(".").pop() || "mp4";
        const uniqueFileName = `actor-videos/${ctx.user.id}/${timestamp}-${input.fileName}`;
